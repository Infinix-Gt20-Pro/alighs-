...
async function deployHardenedSchema() {
  console.log('Deploying hardened schema, constraints, sequence, and atomic function...');

  const sql = `
  -- 1. Ensure Sequence
  CREATE SEQUENCE IF NOT EXISTS public.order_counter_seq START WITH 100;

  -- 2. Ensure Constraints on Products
  ...
  -- 3. Ensure Constraints on Orders
  ...
  ALTER TABLE public.orders
    ADD CONSTRAINT chk_order_status CHECK (order_status IN ('Pending', 'Confirmed', 'Processing', 'Packed', 'Shipped', 'Out for Delivery', 'Delivered', 'Cancelled', 'Returned')),
    ADD CONSTRAINT chk_payment_status CHECK (payment_status IN ('Pending', 'Paid', 'Failed', 'Refunded', 'COD')),
    ADD CONSTRAINT chk_payment_method CHECK (payment_method IN ('COD', 'UPI', 'ONLINE'));

  -- 4. B-Tree Indexes for High Performance
  ...
  -- 5. Atomic Order Creation Stored Function
  ...
  BEGIN
    ...
    -- 5. Insert Order FIRST (foreign key target)
    INSERT INTO public.orders (
      id, order_number, customer_id, subtotal, discount, shipping_charge, total_amount,
      payment_method, payment_status, order_status, customer_notes,
      razorpay_order_id, razorpay_payment_id, created_at, updated_at
    ) VALUES (
      v_order_id, v_order_number, v_customer_id, v_subtotal, 0, v_shipping, v_total,
      v_payment_method, v_payment_status, v_order_status, v_customer_notes,
      v_razorpay_order_id, v_razorpay_payment_id, NOW(), NOW()
    );

    -- 6. Insert Order Items (Order already exists, foreign key satisfied)
    FOR v_item IN SELECT * FROM jsonb_to_recordset(p_order_payload->'items') AS x(product_id TEXT, quantity INT)
    LOOP
    ...
    VALUES (
      ...
      v_order_status,
      NOW(),
      'Order placed via ' || v_payment_method || '. Authoritative server validation complete.'
    );
  ...
  console.log('Schema Hardening Result:', JSON.stringify(res, null, 2));
}

deployHardenedSchema().catch(console.error);
